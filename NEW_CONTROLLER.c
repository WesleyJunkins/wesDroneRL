 ...
 */
 ...
 int main(int argc, char **argv) {
   ...
   printf("Start the drone...\n");
     ...
     // Print IMU values at startup
   const double *rpy = wb_inertial_unit_get_roll_pitch_yaw(imu);
   printf("IMU at startup: roll=%.4f, pitch=%.4f, yaw=%.4f\n", rpy[0], rpy[1], rpy[2]);
   ...
   const double k_vertical_offset = 0.2;   // Vertical offset where the robot actually targets to stabilize itself.
   //const double k_vertical_p = 3.0;        // P constant of the vertical PID.
   const double k_roll_p = 5.0;           // P constant of the roll PID. 50.0
   const double k_pitch_p = 5.0;          // P constant of the pitch PID. 30.0
 
   // Variables.
   double target_altitude = 1.0;  // The target altitude. Can be changed by the user.
   double target_x = 0.0;
   double target_z = 0.0;
   double target_yaw = 0.0;  // Changed from -1 to 0 to maintain current heading
   double kp_x = 1.0;
   double kd_x = 1.0;
   double kp_z = 1.0;
   double kd_z = 1.0;
   ...
   while (wb_robot_step(timestep) != -1) {
     ...
     const double yaw_acceleration = wb_gyro_get_values(gyro)[2];  // Added yaw rate
     
     // Set initial position targets to current position to prevent unwanted movement
     if (time < 2.0) {  // Only set targets in the first 2 seconds
       target_x = position_x;
       target_z = position_z;
     }
     ...
     double cam_roll_pos = CLAMP(-0.115 * roll_acceleration, -0.5, 0.5);
     double cam_pitch_pos = CLAMP(-0.1 * pitch_acceleration, -0.5, 0.5);
     ...
     pitch_input = CLAMP(pitch_input, -10.0, 10.0);
     roll_input = CLAMP(roll_input, -10.0, 10.0);
     vertical_input = CLAMP(vertical_input, -10.0, 10.0);
     const double clamped_yaw_input = CLAMP(yaw_input, -5.0, 5.0);
     ...
     const double front_left_motor_input = k_vertical_thrust + vertical_input - roll_input + pitch_input - clamped_yaw_input;
     const double front_right_motor_input = k_vertical_thrust + vertical_input + roll_input + pitch_input + clamped_yaw_input;
     const double rear_left_motor_input = k_vertical_thrust + vertical_input - roll_input - pitch_input + clamped_yaw_input;
     const double rear_right_motor_input = k_vertical_thrust + vertical_input + roll_input - pitch_input - clamped_yaw_input;
